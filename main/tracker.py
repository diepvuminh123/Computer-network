...
def ClientList():
    ...
    ping = Button(window, text="Ping", font=('Segoe UI',17), fg="#800020", bg="#FFFFF0", width=10, height=1, command=lambda: Ping(ping_ip_var))
    ping.place(x=50, y=20)

    discover_ip = Entry(window, textvariable=dis_ip_var, width=15, bd=4, font=('Arial', 14)).place(x=240, y=102)
    discover = Button(window, text="Detail", font=('Segoe UI',17), fg="#800020", bg="#FFFFF0", width=10, height=1, command=lambda: Discover(dis_ip_var))
...
def Server():
    ...
    server.bind(ADDR)
    server.listen()
    res = f"[STARTING] Server has run\n[LISTENING] Server is on {IP}:{PORT}.\n"
    box.insert(INSERT, res)
...
start_button = Button(root, text="START SERVER", font=('Segoe UI', 17),
                      bg="#FFFFF0", fg="#800020", activebackground="#005BB5", activeforeground="white", 
                      command=ServerThread)
start_button.place(x=140, y=20)

stop_button = Button(root, text="STOP SERVER", font=('Segoe UI', 17),
                     bg="#FFFFF0", fg="#cc0000", activebackground="#B50000", activeforeground="white", 
                     command=StopServer)

clist_button = Button(root, text="CLIENT LIST", font=('Segoe UI', 17),
                      bg="#FFFFF0", fg="#800020", activebackground="#005BB5", activeforeground="white", 
                      command=startClientList)
clist_button.place(x=157, y=90)
